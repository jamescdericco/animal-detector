...
def shrink_image(image_path):
    ...
    with Image.open(image_path) as img:
        ...
        if (
            max(width, height) > MODEL_INPUT_SIZE
            and min(width, height) >= MODEL_INPUT_SIZE
        ):
            ...
            print(
                f"Skipped {image_path}, already {MODEL_INPUT_SIZE}x{MODEL_INPUT_SIZE} px"
            )
        else:
            print(
                f"Image {image_path} is too small at {width}x{height}. Resizing images smaller than {MODEL_INPUT_SIZE} px in width or height is not supported."
            )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Reduce the size of .png images to the model input tensor dimensions adjusting aspect ratio with center-cropping."
    )
    parser.add_argument(
        "root_directory", help="The root directory to search for PNG images."
    )
