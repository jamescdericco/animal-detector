...
notify_txt_phone_numbers_list = notify_txt_phone_numbers.split(',') if notify_txt_phone_numbers is not None else []
...
def notify_animal_detected(prediction, confidence, frame_datetime):
    ...
    if prediction not in detection_times or (frame_datetime - detection_times[prediction]).seconds > 30 * 60:
        detection_times[prediction] = frame_datetime
        for phone_number in notify_txt_phone_numbers_list:
            send_txt(phone_number, f'At {frame_datetime} detected {prediction} {confidence_percent:.2f}%')
